...
tol = 0.0001; % Setting the tollerence required between the input and output 
    % value of a, adash to finish the optimisation
...
loopCountMax = 500; %Define the maximum numebr of loops permetted whilst 
    % solving for both a and adash to stop infinate looping
...
while Error > tol
    ...
    Vrel = ((V0*(1-a))^2 + (omega*y*(1+adash))^2)^0.5; %Calculating the relative 
        % velociy of the airflow. m/s

    Re = (rho*Vrel*Chord)/mew; % Calculating the reynolds number.
    [Cl, Cd] = ForceCoefficient(alpha, Re); % Running function to calculate 
    ...
    adashNew = 1/(((4*F*(sin(phi)*cos(phi)))/(sigma*Ct))-1); % Calcualting the 
        % new value of adash
    ...
    if loopCount < loopCountMax %If loop count is less than the desired maximum
        ...
        if(loopCount<relaxationLoop)
            ...
            k=0.1; % For higher loopcounts set relaxation factor to 0.1 and 
                % start to implement to improve stability.
        end
        
        Error = abs(aNew-a)+abs(adashNew-adash); % Calculating the difference 
            % between the input and output values of a, adash
        a = k*(aNew-a)+a; % adding a relaxation factor to the value of a to 
            % help avoid an unstable loop
        
        if a>1 % Check if the value of a is larger than one
            a=1; % Set the value ot 1 to avoid complex F value
        end
        
        adash = k*(adashNew-adash)+adash; % adding a relaxation factor to the 
            % value of adash to help avoid an unstable loop
    ...
    elseif loopCount > loopCountMax && loopCount < 2*loopCountMax % If loop count 
        ...
        Error = abs(aNew-a); % Calculating the difference between the input 
            % and output values of a, adash
        a = k*(aNew-a)+a; % adding a relaxation factor to the value of a to 
            % help avoid an unstable loop
        ...
        adash = 0; %set adash to 0 if the maximum number of desired loopes 
            % has been exceeded
